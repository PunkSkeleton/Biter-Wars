...
function selectBitersInner(area, force, offset) 
	...
	if force == "south" then
		opposingForce = "north"
		--game.write_file("biter-wars.log", "a\n", true)
	else
		opposingForce = "south"
		--game.write_file("biter-wars.log", "b\n", true)
	end
	--game.write_file("biter-wars.log", "c\n", true)
	game.write_file("biter-wars.log", "Selecting biters!" .. " middle: " .. middleX .. ":" .. middleY .. "force: " .. force .. "\n", true)
	...
	if individualBiters == nil then
		--game.write_file("biter-wars.log", "individualBiters is nil!\n", true)
		return nil
	end
	if #individualBiters == 0 then
		--game.write_file("biter-wars.log", "individualBiters size is zero!\n", true)
		return nil
	end
...
function advanceBiters(biterGroup, forceName, startPosX, startPosY)
	...
	game.write_file("biter-wars.log", "Advancing biters from position: \n" .. startPosX .. ":" .. startPosY, true)
	...
	game.write_file("biter-wars.log", "Distances: \n" .. distanceX .. ":" .. distanceY, true)
...
function aiRootStep()
	...
	profiler.stop()
	game.write_file("biter-wars.log", "aiStep took: ", true)
	game.write_file("biter-wars.log", {"", profiler}, true)
	game.write_file("biter-wars.log", "\n", true)
	profiler.reset()
end
